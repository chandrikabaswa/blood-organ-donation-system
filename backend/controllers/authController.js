...
const register = async (req, res) => {
  try {
    ...
    if (role === 'donor') {
      ...
      profile = mockStore.createHospital({
        userId: user._id,
...
const login = async (req, res) => {
  try {
    ...
    const user = mockStore.findUserByEmail(email.trim());
    if (!user) {
    ...
    let profile = null;
    if (user.role === 'donor') {
      profile = mockStore.findDonorByUserId(user._id);
    } else if (user.role === 'hospital') {
      profile = mockStore.findHospitalByUserId(user._id);
    }
